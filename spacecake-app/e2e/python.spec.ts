...
test.describe("python e2e", () => {
  ...
  test("open workspace and render core.py rich", async ({ electronApp, tempTestDir }) => {
    ...
    await expect(window.getByText("class", { exact: true }).first()).toBeVisible()
    ...
    const activeGutter = firstEditor
      .locator(".cm-gutter.cm-lineNumbers .cm-gutterElement.cm-activeLineGutter")
      .first()
    ...
    const contentTextLocators = firstEditor.locator(".cm-content").getByText("datetime")
    ...
    const selectionBackgrounds = firstEditor.locator(".cm-selectionLayer .cm-selectionBackground")
    ...
    const newPara = window.locator(".ContentEditable__root").getByRole("paragraph").first()
    await expect(newPara).toBeVisible()
    await window.keyboard.press("ArrowDown", { delay: 100 })

    // recreate a spacer and then type to verify text stays
    await thirdEditorRoot.locator(".cm-content").getByText("Person").first().click()
    ...
    await window.getByTestId("lexical-editor").pressSequentially(spacerText1, { delay: 100 })
    ...
    const fibonacciBlock = window.locator('[data-block-id="fibonacci-function"]')
    await fibonacciBlock.locator(".cm-content").click()
    await expect(fibonacciBlock.locator(".cm-editor").first()).toHaveClass(/focused/)
    ...
    await window.getByTestId("lexical-editor").pressSequentially(spacerText3, { delay: 100 })
    ...
    await window.getByTestId("lexical-editor").pressSequentially(keepText, { delay: 100 })
    ...
    const importBlockToDelete = window.locator('[data-block-id="anonymous-import"]')
  ...
  test("switching between core.py and empty.py updates editor", async ({
    ...
    await expect
      .poll(async () => window.evaluate(() => document.querySelector("header")?.textContent || ""))
      .toContain("empty.py")
  ...
  test("saving new misc block persists after rerender without navigation", async ({
    ...
    const xEditor = window.locator(".cm-editor").filter({ hasText: "x = 5" }).first()
