...
const makeLayerFromEnv = (env: AppEnv) =>
  Layer.effect(
    SpacecakeHome,
    makeSpacecakeHome as unknown as Effect.Effect<
      SpacecakeHome,
      never,
      AppEnvTag
    >
...
describe("SpacecakeHome — path computation", () => {
  ...
  it.scoped("cliBinDir is appDir/bin when not packaged", () =>
    Effect.gen(function* () {
      ...
      const home = yield* SpacecakeHome.pipe(Effect.provide(layer))
      expect(home.cliBinDir).toBe(
        path.join(tempDir, ".spacecake", ".app", "bin")
      )
    }).pipe(Effect.provide(NodeFileSystem.layer))
  )
  ...
  it.scoped(
    "bundledCliBinaryPath is resourcesPath/bin/spacecake when packaged",
    () =>
      Effect.gen(function* () {
        const effectFs = yield* EffectFileSystem.FileSystem
        const tempDir = yield* effectFs.makeTempDirectoryScoped()
        ...
        const home = yield* SpacecakeHome.pipe(Effect.provide(layer))
        expect(home.bundledCliBinaryPath).toBe(
          path.join(resPath, "bin", "spacecake")
        )
      }).pipe(Effect.provide(NodeFileSystem.layer))
  )
  ...
  it.scoped("writes getting-started.md with correct content", () =>
    Effect.gen(function* () {
      ...
      const content = fs.readFileSync(
        path.join(homeDir, ".app", "getting-started.md"),
        "utf-8"
      )
  ...
  it.scoped("writes statusline.sh with executable permissions", () =>
    Effect.gen(function* () {
      ...
      const stat = fs.statSync(
        path.join(homeDir, ".app", "hooks", "statusline.sh")
      )
...
describe("installCli — dev mode", () => {
  ...
  it.scoped("wrapper content contains correct cliSourceEntryPath", () =>
    Effect.gen(function* () {
      ...
      const content = fs.readFileSync(
        path.join(homeDir, ".app", "bin", "spacecake"),
        "utf-8"
      )
...
describe("installCli — packaged mode", () => {
  it.scoped(
    "creates symlink at globalBinTarget pointing to bundledCliBinaryPath",
    () =>
      Effect.gen(function* () {
        const effectFs = yield* EffectFileSystem.FileSystem
        const tempDir = yield* effectFs.makeTempDirectoryScoped()
        ...
        expect(fs.readlinkSync(symlinkTarget)).toBe(
          path.join(resPath, "bin", "spacecake")
        )
      }).pipe(Effect.provide(NodeFileSystem.layer))
  )
