...
export type InitialContent = {
  viewKind: ViewKind
  content: { kind: "state"; data: EditorCache } | { kind: "file"; data: EditorFile }
}
...
export class EditorManager extends Effect.Service<EditorManager>()("EditorManager", {
  effect: Effect.gen(function* () {
    ...
    const readEditorState = (filePath: AbsolutePath, editorId?: EditorPrimaryKey) =>
      Effect.gen(function* () {
    ...
    const readFile = (filePath: AbsolutePath) =>
      Effect.gen(function* () {
        const fileContent = yield* Effect.promise(() => window.electronAPI.readFile(filePath))
    ...
    const readStateOrFile = (props: {
      ...
      Effect.gen(function* () {
        const maybeState = yield* readEditorState(props.filePath, props.editorId)

        if (isRight(maybeState) && maybeState.value.state) {
          // if no targetViewKind specified, or it matches the stored viewKind,
          // return the stored state
          if (!props.targetViewKind || props.targetViewKind === maybeState.value.viewKind) {
        ...
        return left<PgliteError | FileSystemError | EditorManagerError, InitialContent>(
