...
description: Display Claude Code session metrics (model, context usage, cost) in your `spacecake` header.
---

## overview

`spacecake` can display live Claude Code session metrics in the terminal header badge, including:
...
the integration has zero impact on Claude Code if `spacecake` isn't running—the hook simply fails silently.

## ⚙️ troubleshooting

**no data appearing in the header badge?**
