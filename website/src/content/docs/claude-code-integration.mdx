...
run `claude` from the integrated terminal — it automatically connects to spacecake, giving Claude context about your open files. a common workflow is to run Claude Code in one [terminal tab](/terminal#tabs) while keeping a separate tab for your dev server or manual commands.
...
Claude Code can enter plan mode on its own when it determines a task benefits from upfront planning. in plan mode, Claude creates a plan file before making changes, giving you a chance to review and edit the approach.
